...
export const onTopUpRequestCreate = onDocumentCreated("topUpRequests/{requestId}", async (event) => {
    ...
    try {
        ...
        // If pujaseraGroupSlug exists, it's a tenant top-up, notify pujasera admin.
        // Otherwise, it's a direct store/pujasera top-up, notify platform superadmin.
        const targetAdminIdentifier = pujaseraGroupSlug ? `pujasera_admin_${pujaseraGroupSlug}` : 'platform_superadmin';
        ...
        logger.info(`Queued new top-up request notification for target: ${targetAdminIdentifier}.`);
